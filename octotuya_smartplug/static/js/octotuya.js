/*
 * View model for OctoTuya-SmartPlug
 *
 * Author: ziirish
 * License: AGPLv3
 */
$(function() {
    function octotuyaViewModel(parameters) {
		...
		self.onBeforeBinding = function() {
			self.arrSmartplugs(self.settings.settings.plugins.octotuya.arrSmartplugs());
        }
		...
		self.addPlug = function() {
			...
			self.settings.settings.plugins.octotuya.arrSmartplugs.push(self.selectedPlug());
			$("#TuyaPlugEditor").modal("show");
		}

		self.removePlug = function(row) {
			self.settings.settings.plugins.octotuya.arrSmartplugs.remove(row);
		}
		...
		self.onDataUpdaterPluginMessage = function(plugin, data) {
			...
			plug = ko.utils.arrayFirst(self.settings.settings.plugins.octotuya.arrSmartplugs(),function(item){
				...
				switch(data.currentState) {
					...
					default:
						new PNotify({
							...
							text: 'Status ' + plug.currentState() + ' for ' + plug.ip() + '. Double check IP Address\\Device ID\\Local Key in OctoTuya-SmartPlug Settings.',
							type: 'error',
							hide: true
							});
...
/* 			if(data.sysCmdOn()){
		...
		self.sendTurnOn = function(data) {
            $.ajax({
                url: API_BASEURL + "plugin/octotuya",
                type: "POST",
                dataType: "json",
                data: JSON.stringify({
                    command: "turnOn",
    	...
    	self.turnOff = function(data) {
			if((data.displayWarning() || (self.isPrinting() && data.warnPrinting())) && !$("#octotuyaWarning").is(':visible')){
				self.selectedPlug(data);
				$("#octotuyaWarning").modal("show");
			} else {
				$("#octotuyaWarning").modal("hide");
/* 				if(data.sysCmdOff()){
...
/* 		self.sysCommand = function(sysCmd) {
		...
		self.checkStatuses = function() {
			ko.utils.arrayForEach(self.settings.settings.plugins.octotuya.arrSmartplugs(),function(item){
				if(item.label() !== "") {
					console.log("checking " + item.label())
					self.checkStatus(item.label());
				}
			});
			if (self.settings.settings.plugins.octotuya.pollingEnabled()) {
				setTimeout(function() {self.checkStatuses();}, (parseInt(self.settings.settings.plugins.octotuya.pollingInterval(),10) * 60000));
			};
        };
    }
    ...
    OCTOPRINT_VIEWMODELS.push([
        ...
        // "#navbar_plugin_octotuya","#settings_plugin_octotuya"
        ["#navbar_plugin_octotuya","#settings_plugin_octotuya"]
    ]);
});
